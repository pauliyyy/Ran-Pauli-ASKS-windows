...
Markdown 版是默认在线阅读入口。PDF 保留用于下载和打印，不依赖 GitHub 的 PDF
预览器。

论文侧重阐述学术思想、方法论与实验依据。中文说明更侧重项目介绍和传播，帮助
...
共同构成一个 Raw 来源；配套元数据保存哈希、提取来源和逐字段复核记录，不作为
第三份事实材料。已有 OCR 回执可以复用，无需再次上传；远程识别始终需要显式授权，
OCR 模型可独立配置。

在生成 Wiki 和图关系前，先评估材料风险并核对关键字段，Agent 与人工复核分别记录。
关键字段未核对时阻止摄入；非关键不确定项可以随已完成的来源包保留。确认日期栏
为空白不等于应补填日期；签名者未确认时，不建立具名负责人关系，也不自动给用户
新增待办。图片页保留 OCR 来源类型、复核限制和保守的可信度。

Agent 模式由当前宿主控制流程，API 模式由程序控制，两者共用校验与受管提交内核。
只有最终校验通过、完成状态持久化后才清理来源；清理失败可以单独重试，不重复摄入。
...
带日期的 [ASKS 中文介绍](docs/introduction/ASKS-Chinese-Introduction-2026-09-03.md)
是面向一般使用者的项目介绍和传播材料。初版内容随 `v0.2.2` 发布，`v0.2.3`
提供了用户重新生成的正确 PDF 版式。2026年9月3日修订版将工程说明同步到
`v0.3.0`；2026年9月4日更新随 `v0.4.0` 发布，涵盖受控漫画生成、Agent 摄入恢复、
显式会议来源证据规则和子 Hub 特异性路由，同时不改变实验范围、arXiv 版本或冻结
论文数据产物。详细边界见
[版本对应关系](docs/introduction/README.md)；需要打印时可
[下载 PDF](docs/introduction/ASKS-Chinese-Introduction-2026-09-03.pdf)。
...
不会静默重跑整条摄入事务。Agent 后端在语义阶段使用带
`prompt + write_to + transaction_id` 的类型化交接，并从同一事务恢复；
`agent_required`、`partial` 等可继续状态不会被误报为普通进程错误。具有强会议
速记特征的办公文档保留语音识别来源，部门和负责人关系必须有来源原文明确支持。
...
`--allow-remote`；详见 `operations/COMIC_GENERATION.md`。
随后阅读 `AGENTS.md`：
