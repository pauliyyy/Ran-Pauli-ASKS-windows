...
项目中文介绍，2026年9月3日发布，2026年9月4日同步至 Ran-ASKS v0.4.0。本页是 GitHub 在线阅读版，PDF 保留用于下载和打印。
...
| **显式远程生成** | 漫画图片只通过登记模型、明确输出目录和逐次远程授权生成，并保留可审计回执。 |

### 统一摄入、局部恢复与模型分工

论文、会议纪要和普通文档可以使用不同的语义协议，但校验后的语义统一进入 Knowledge IR、确定性 graph plan 和唯一事务性写图器。按来源保存的描述与来源关系参与身份匹配、导航、清理和 Hub 维护。Agent backend 由当前宿主 Agent 持有控制循环，脚本只提供 `agent-task-v1`、暂存输出、机械校验和事务提交；API backend 由程序通过 DSH 与受限 Worker 推进。两端共享确定性内核，不共享语义执行 adapter。具有会议速记特征的办公文档保留语音识别来源；部门与负责人关系必须由原文明示。

摄入编排由独立的 INGEST_BACKEND 选择。API 模式还可以通过 INGEST_GENERATION\_\* 和 INGEST_PROPOSITION\_\* 分别指定生成与命题抽取模型；未配置时复用主 LLM。无论语义由当前 Agent 还是 API Worker 完成，持久化都只由共享 validator 和统一事务写入机制控制。
...
查询、摄入、视觉检查、视觉对象化、漫画生成和工程定位分别通过不同的能力入口注册。漫画生成只允许已登记模型和明确的项目 `outputs/`，真实远程调用必须再次显式授权；它生成视觉素材和审计记录，不充当科学事实源。这样的分离让 Agent 能自动选择合适工具，同时避免把不相关的写入能力暴露给普通查询。DSH 只负责工具编排、守卫和会话记录。实际摄入、检索和图操作仍由底层脚本执行，科学事实仍保存在 Raw、Wiki 和 graph.db 所构成的知识底座中。
...
## 2026年9月8日工程更新补充（v0.5.0）

本补充将图片文档摄入与复核机制归入 v0.5.0 新能力边界，不建立新的标签或 Release，不改变既有论文实验、冻结数据产物及其复现边界。PDF 保留原说明版式，并在末尾附上本补充内容。

### 图片也是一个可追溯的来源包

文字型图片通过独立 OCR 工具转写。原图和同名 Markdown 共同组成一个 Raw 来源：原图保留凭证，Markdown 支持检索与精确引用；配套元数据保存哈希、提取方式和复核记录，不成为第三份事实材料。已有识别回执可以复用，远程识别必须显式授权，模型可独立配置。

### 复核说明证据边界，不制造确定性

生成 Wiki 和图关系之前，先评估材料风险，逐项核对金额、编号、审批状态等关键字段。Agent 对照原图的复核与人工确认分别记录；关键字段未核对时阻止摄入。非关键不确定项可以随已完成来源保留，不自动产生用户待办。

例如，确认日期栏确实为空白，是有效的核对结论，不是要求用户补日期；有手写签名但未确认姓名时，可以保留原样而不建立具名负责人关系。摄入完成不代表原件事项已提交、获批或执行。Wiki 会保留 OCR 来源、复核状态和必要限制，而不把图片默认归为官方高可信材料。

### 同一确定性内核，两种控制方式

Agent 模式由当前宿主持有控制循环，API 模式由程序推进，二者共享提取校验、来源绑定、Wiki 与图校验和受管提交机制。原图、转写与来源元数据共同归档；只有最终校验通过并持久化完成状态后，才清理 inbox 原件和事务暂存。清理失败可以单独重试，不重复生成、写图或写入日志。

公开 GitHub 仓库只同步白名单中的通用代码、规范、模板及已批准的冻结产物，不包含个人 Raw、生产 Wiki、图数据库、密钥或活动项目资料。具体命令、复核记录格式和状态解释以仓库中的 `operations/IMAGE_OCR.md` 与 `operations/INBOX.md` 为准。

### 版本随实际公开变化递增

以后每批公开更新由 Agent 按功能与兼容性自动判断级别：兼容修复和小文档更新为 PATCH，兼容新增能力为 MINOR，破坏公开契约为 MAJOR，不按改动行数判断。脚本排号并记录理由、归档 Changelog；校验拒绝同版本或倒退更新。同批次重建不重复升版，创建标签或 Release 另行授权。本次新增图片摄入能力，因此从 0.4.0 升至 0.5.0。

## 2026年9月12日工程更新补充（v0.6.0）

本次以兼容新增能力从 v0.5.0 升至 v0.6.0：对话提交的文档与原生演示文稿接入既有受管摄入内核，不改变冻结论文实验和数据产物，也不创建新标签或 Release。PDF 保留此前版式并追加本补充。

### 对话提交等同于 inbox 摄入，但保留原件

文件附件或文件路径使用 wg.py ingest <path> --keep-source --subproject <domain>；直接粘贴的正文通过标准输入交给 wg.py ingest --stdin --name notes.txt --subproject <domain>。领域按内容选择。两者先建立受管暂存，再复用 inbox 的分类、去重、语义生成、校验、恢复与事务提交。

文件原件即使位于 inbox 也不会因这次摄入被删除；只有事务拥有的暂存副本可在成功后清理。粘贴正文按原始 UTF-8 字节保存，不混入对话指令，也不先摘要。原件保留策略覆盖失败恢复与重复检测；普通 inbox 项的清理规则不变。暂存回执只用于流程恢复，不能代替 Raw 事实来源。

### 演示文稿先逐页核对，再生成知识

原生 PPTX 提取保留页序、文字、表格、对象位置与演讲者备注。语义生成前，当前宿主对照本地渲染的全部页面核对关键字段、表格对应关系以及图片和非文本对象的信息遗漏。关键未决项阻断，非关键限制保留；原件、可读 Markdown 与来源记录在同一事务中归档。

页图与临时 PDF 默认不上传、不进入 Raw；逐页内容复核不等于版式检查，也不调用可编辑 PPT 重建。动画和视频的播放信息不在静态核对保证范围，旧 PPT 文件需要显式转换。Wiki 按主题组织，不把每页幻灯片自动变成独立实体，也不把拟建或申报写成已实施。

### 来源地址和日期必须来自证据

会议标题确定后，程序将 Wiki 的来源字段绑定到最终 Raw 地址，并在提交前拒绝旧目录、缺失或多余来源，正文不被机械改写。通用文档日期依次检查原始文件名、正文明确的成文日期和 inbox 内目录日期；缺少证据时保留未知，不借用摄入日。历史已完成事务不会因代码更新自动重跑。
